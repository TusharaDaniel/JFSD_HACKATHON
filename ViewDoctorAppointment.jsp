		...
		<nav class="navbar navbar-inverse navbar-static-top"
			...
			<div class="container">
				<div class="navbar-header">
					...
					<a class="navbar-brand" href="DoctorHome.jsp"><img
						src="images/log.png" style="height: 80px" alt="Dispute Bills">
					</a>
				</div>
...
<%
	...
	ResultSet rs2 = st2.executeQuery("SELECT count(*) FROM `appointment` WHERE status='Pending' and date_format('"
	+ currentDateTime + "','%Y-%m-%d %H:%i') > appoint_date_time");
	if (rs2.next()) {
		count = rs2.getInt(1);
	}
	...
	for (int i = 0; i < count; i++) {
		...
		x = st1.executeUpdate("update appointment set status ='Cancelled' WHERE status='Pending' and date_format('"
		+ currentDateTime + "','%Y-%m-%d %H:%i') > appoint_date_time");
	}
	if (x > 0) {
	...
	"SELECT count(*) FROM `appointment` WHERE appoint_time > '22:00' or appoint_time <= '09:59' or (status='Pending' and date_format('"
			+ currentDateTime + "','%Y-%m-%d %H:%i') >= appoint_date_time)");
	if (rs3.next()) {
	...
	for (int i = 0; i < counter; i++) {
		...
		z = st4.executeUpdate(
		"update appointment set status ='Cancelled' WHERE appoint_time > '22:00' or appoint_time <= '09:59' or (status='Pending' and date_format('"
				+ currentDateTime + "','%Y-%m-%d %H:%i') >= appoint_date_time)");
	}
	if (z > 0) {
...
<table id="myTable" class="table table-striped table-bordered">
	...
	<tbody>
		...
		%>
		<tr>
			<td><%=rss.getString("pid")%></td>
			<td><%=rss.getString("pname")%></td>
			<td><%=rss.getString("appoint_date")%></td>
			<td><%=rss.getString("appoint_time")%></td>
			<td><%=rss.getString("status")%></td>
			<td><a
				href="AlterAppointment?did=<%=rss.getString("did")%>&&pid=<%=rss.getString("pid")%>&&date=<%=rss.getString("appoint_date_time")%>&&status=<%=status%>">Confirm</a>&nbsp;||&nbsp;<a
				href="AlterAppointment?did=<%=rss.getString("did")%>&&pid=<%=rss.getString("pid")%>&&date=<%=rss.getString("appoint_date_time")%>&&status=<%=status1%>">Cancel</a></td>
		</tr>
			<%
				}
			} else {
